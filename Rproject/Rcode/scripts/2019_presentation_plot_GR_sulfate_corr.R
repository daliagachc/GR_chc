#Makes scatterplots of growthrates for binned sizegroups vs sulfate concentrations.

#------------------------------DEFINITIONS------------------------------

maxMad=3.5
minObs=8
sel_ions=c("positive","negative")
...
temporal_function_plot_cell=function(df,sizegroup,mag){
  ...
  plot=ggplot(data=subset(help,GR_madDist <= maxMad &!is.na(sizeGroups)),
              aes(x=value,y=GR))+
    geom_point()+
    scale_x_log10()+
...
wd=wd%>%
  ...
  dplyr::filter(charge %in% sel_ions)


#wd$Time=force_tz(wd$Time,tzone="etc/GMT+4")
wd=wd%>%
  mutate(sizeGroups=cut(dp_splined,breaks=c(1.3,3,7,20),include.lowest=T),
...
p1=temporal_function_plot_cell(wd3,"[1.3,3]","Sulfate\n(ACSM)")+
  labs(x="Sulfate [ug/m3]",y="GR [nm/h]",title="(1.3,3] nm")
p2=temporal_function_plot_cell(wd3,"(3,7]","Sulfate\n(ACSM)")+
  labs(x="Sulfate [ug/m3]",y="GR [nm/h]",title="(3,7] nm")
p3=temporal_function_plot_cell(wd3,"(7,20]","Sulfate\n(ACSM)")+
  labs(x="Sulfate [ug/m3]",y="GR [nm/h]",title="(7,20] nm")
