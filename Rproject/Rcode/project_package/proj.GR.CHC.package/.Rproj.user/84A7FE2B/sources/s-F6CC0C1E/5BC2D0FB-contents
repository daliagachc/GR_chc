#' Imports NAIS
#'
#' Import NAIS data (.sum) from the NAIS3 in CHC
#'
#' @details These files are big, it might take a while.
#'
#' @param year The year to be imported. (You can import one year only)
#' @param where Path to the containing folder. All files with extension .sum within the path will be pulled.
#'
#' @return data table of data
#' @export
#'
#' @examples import_NAIS_data_2013_2018(folderpath,2016)
import_NAIS_data_2013_2018=function(where,year){
  yearsel=year
  ...
  tempall = list.files(path=where,pattern="*.sum")
  ...
  for (i in 1:length(temp)){
    filename=temp[i]
    wd=fread(paste(where,temp[i],sep="/"),header=F,skip=1)
    headers=names(fread(paste(where,temp[i],sep="/"),header=T,skip=0))
