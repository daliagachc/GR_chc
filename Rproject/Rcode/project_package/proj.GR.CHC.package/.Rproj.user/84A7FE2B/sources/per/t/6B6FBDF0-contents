#' import GR mode data
#'
#' imports data for the size growing of the growing mode in time. Saltena campaign.
#' Can save to file if desired.
#' All files within the folder will be loaded
#'
#'
#' @param where path to folder containing the files
#' @param writeFile if TRUE, write to file.
#'
#' @return data frame with the growing mode information
#' @export
#'
#' @examples to be filled
import_GR_mode_dat=function(where,writeFile = FALSE){
  #creating a list pointing to all files with extension .dat
  temp = list.files(path=where,pattern="*.dat")
  ...
  for (i in 1:length(temp)){
    import=readMat(paste(where,temp[i],sep="/"))
