...
posindices=which((data.frame(date=NAIS_ion_list_indices,flag=F) %>%
  mutate(flag=grepl("p",date)))$flag)
negindices=which((data.frame(date=NAIS_ion_list_indices,flag=F) %>%
                    mutate(flag=grepl("n",date)))$flag)

helplist=NAIS_ion_list
...
NAIS_ion_df=do.call("rbind",helplist)
remove(helplist)
