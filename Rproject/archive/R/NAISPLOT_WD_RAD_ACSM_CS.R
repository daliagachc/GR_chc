...
time1=as.POSIXct("2018-04-06",tz="etc/GMT+4")
time2=as.POSIXct("2018-04-18",tz="etc/GMT+4")
...
wd=subset(wd,magnitude %in% c("Nitrate","Organics","Sulfate"))
wd.normfacs=wd %>%
...
CSplot=ggplot(data=wd)+
  geom_line(aes(x=localTime,y=V2))+
  scale_x_datetime(date_breaks = "1 day",
                   labels = date_format("%d", tz="etc/GMT+4"),
                   position = "bottom",
                   expand = c(0.01,0.01))+
  labs(y="CS")+
  scale_y_continuous(trans="log10",limits=c(0.5e-3,3e-2))
...
CSplot=CSplot+theme(axis.title.x=element_blank(),
...
ptotal=ggarrange(negplot,acsmplot,CSplot,wdplot,radplot,
          ncol = 1, nrow  =5,align="v",
          heights=c(1,1,0.5,0.75,0.35))
